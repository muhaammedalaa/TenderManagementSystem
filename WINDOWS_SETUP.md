# 🪟 TMS Windows Setup Guide

دليل شامل لتشغيل نظام إدارة المناقصات (TMS) على Windows باستخدام Docker.
...
- تأكد من تفعيل Hyper-V (إذا كان متاحاً)

### 2. Git for Windows
- تحميل من: https://git-scm.com/download/win
- اختر "Git Bash" أثناء التثبيت

### 3. Windows Terminal (اختياري)
- تحميل من: Microsoft Store
- يوفر تجربة أفضل مع Docker
...
### الطريقة الثانية: استخدام Git Bash
...
make -f Makefile.windows dev-logs   # سجلات التطوير
make -f Makefile.windows dev-down   # إيقاف التطوير
```

### أوامر الإنتاج
```bash
make -f Makefile.windows prod       # وضع الإنتاج
make -f Makefile.windows prod-logs  # سجلات الإنتاج
make -f Makefile.windows prod-down  # إيقاف الإنتاج
```
...
make -f Makefile.windows db-reset   # إعادة تعيين (حذف كل البيانات)
```
...
### مشكلة: Database connection issues
```cmd
# الحل:
...
# 3. أعد تعيين قاعدة البيانات (سيحذف كل البيانات)
make -f Makefile.windows db-reset
```
...
- أضف Docker Desktop إلى الاستثناءات
